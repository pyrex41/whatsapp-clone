...
let appReducer: Store<AppState, AppAction>.Reducer = { state, action, environment in
    ...
    case .onAppear:
        ...
        return .run { send in
            do {
                ...
                await send(.threadsLoaded(.success(threads)))
            } catch {
                await send(.threadsLoaded(.failure(error)))
            }
        }
    ...
    case let .threadsLoaded(result):
        ...
        case let .success(threads):
            ...
            if let firstThread = threads.first {
                ...
                return .run { send in
                    await send(.loadMessages(firstThread.id))
                }
            }
            return .none
    ...
    case let .threadSelected(threadID):
        ...
        commands.append(
            .run { send in
                await send(.loadMessages(threadID))
            }
        )
        commands.append(
            .run { send in
                await environment.realtime.connect(threadID) { message in
                    await send(.receiveRealtimeMessage(message))
                }
            }
        )
    ...
    case .createThread:
        ...
        return .run { send in
            do {
                ...
                await send(.threadCreated(.success(thread)))
            } catch {
    ...
    case let .threadCreated(result):
        ...
        case let .success(thread):
            ...
            return .run { send in
                await send(.loadMessages(thread.id))
            }
    ...
    case let .loadMessages(threadID):
        ...
        return .run { send in
            do {
                ...
                await send(.messagesLoaded(threadID, .success(messages)))
            } catch {
                await send(.messagesLoaded(threadID, .failure(error)))
            }
        }
